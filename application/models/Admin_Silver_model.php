...
class Admin_Silver_model extends CI_Model {

	function ambil_data(){
		$query = $this->db->query("select id_penyewa, nama_instansi,nohp,email, scan from peminjam where jenisPaket=2");
		return $query;
	}	
	...
	public function all($column)
    {
        ...
        $penyewa = $this->db->query("SELECT @rownum := @rownum + 1 AS rank, $column FROM peminjam, (SELECT @rownum := 0) r where jenisPaket=2");
    ...
    public function get_silver(){
  		$query = $this->db->query("SELECT transaksi.*, ruang.*, peminjaman_ruang.*, peminjam.* from transaksi, ruang, peminjam, peminjaman_ruang where peminjam.id_penyewa=peminjaman_ruang.id_penyewa and transaksi.kode_booking=peminjaman_ruang.kode_booking and ruang.id_ruang=peminjaman_ruang.id_ruang and peminjam.jenisPaket=2");
      return $query->result();
	  }
