...
class Admin_MemberSilver extends CI_Controller {
	...
	public function __construct()
    {
        ...
        $this->load->model('Admin_Silver_model');
	...
	{
		$data['peminjam'] = $this->Admin_Silver_model->ambil_data()->result();
		...
		$this->load->view('admin_MemberSilver',$data);
	}

	public function export()
    {
        $this->phpexcel->setActiveSheetIndex(0);
        $this->phpexcel->getActiveSheet()->setTitle('List Penyewa Silver');

        $column = 'nama_instansi, nama, nohp, email';

        $users = $this->Admin_Silver_model->all($column);
        ...
        $filename= 'List Penyewa Silver.xls'; //save our workbook as this file name
  ...
  public function ubahKataSandiPeminjam(){
        ...
        $passBaru = "eds_".$this->Admin_Silver_model->random_sandibaru(4);
        $this->Admin_Silver_model->ubahKataSandiPeminjam($email,$passBaru);
        $this->session->set_flashdata('sukseskatasandiUbah',true);
      //  $data = $this->session();
        $data['error'] = "";
       // $data['peminjam1'] = $this->Admin_MemberGold_model->get_data_peminjam();
        $data['pass'] = $this->Admin_Silver_model->random_katasandi(8);
        $data['sandi'] = $passBaru;
    ...
    function hapus1($id_penyewa){
        ...
        $this->Admin_Silver_model->hapus_data($where,'peminjam');
        redirect('Admin_MemberSilver');
    }
    ...
    public function syllabus_dwp(){
        ...
        $data = array(
        'page' => 'admin_MemberSilver',
        'result' => $this->Admin_Silver_model->get_syllabus_dwp()
        );
        $this->load->view('admin_MemberSilver',$data);
    }
}
